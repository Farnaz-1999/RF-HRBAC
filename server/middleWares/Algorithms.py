...
def chkParentalSpecialPrivileges(parent_privilege,child_privilege):
    ...
    r4srParents=FRHRBAC_db.Roles.find({"name":{"$in":child_privilege["R4SR"]}})
    r4srParents=serializeListRoles(r4srParents)
    r4swParents=FRHRBAC_db.Roles.find({"name":{"$in":child_privilege["R4SW"]}})
    r4swParents=serializeListRoles(r4swParents)
    c4rParents=FRHRBAC_db.Roles.find({"name":{"$in":child_privilege["C4R"]}})
...
def repairFromParent(roleName,ancestors):
    mdb=Mdb()
    FRHRBAC_db=mdb.get_FRHRBAC_db()
    role=FRHRBAC_db.Roles.find_one({"name":roleName})        
    try:
        ancestorRole=FRHRBAC_db.Roles.find_one({"name":ancestors[0]})
        key = os.getenv("KEY")
        ...
        FRHRBAC_db.Roles.update_one({"name":roleName},{"$set":{"privileges":role_privilege}})
    except:
        if len(ancestors)>1:
            ...
            ancestorRole=FRHRBAC_db.Roles.find_one({"name":"rootBackup"})
            key = os.getenv("KEY")
            privileges=decrypt_privileges(ancestorRole["privileges"],key)
            role_privilege=ancestorRole["children"][str(role["id"])]
            role_privilege=decrypt_privileges(role_privilege,key)
...
def sub_hchk(role,children_privilege,privileges,FRHRBAC_db,key):
    sw=0
    for childID,child_privilege in role["children"].items():   
        if(type(child_privilege)==dict):
            role=FRHRBAC_db.Roles.find_one({"id":int(childID)})
            privileges=decrypt_privileges(role["privileges"],key)
            sub_hchk(role,children_privilege,privileges,FRHRBAC_db,key)
            ...
            childRole=FRHRBAC_db.Roles.find_one({"id":int(childID)})            
            children_privilege.update({childID:hchk(childRole["name"],key)})
        else:
            sw=1
            FRHRBAC_db.Roles.delete_one({"id":int(childID)})
    return sw
...
def hchk(roleName,key):
    ...
    sw=sub_hchk(role,children_privilege,privileges,FRHRBAC_db,key)

    if sw:
        newKey=keyGenerator()
        newRolePrivilege=encrypt_privileges(privileges,newKey)
        FRHRBAC_db.Roles.update_one({"id":role["id"]},{"$set":{"children":children_privilege,"privileges":newRolePrivilege}})
